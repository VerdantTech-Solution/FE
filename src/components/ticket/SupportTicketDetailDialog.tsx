...
import {
  ...
  type ProcessRefundRequestPayload,
} from "@/api/cashout";
import {
  getExportedIdentityNumbersByOrderDetailId,
...
export const SupportTicketDetailDialog = ({
  ...
  const [status, setStatus] =
    useState<ProcessTicketRequest["status"]>("InReview");
  ...
  const [isRefundSuccessDialogOpen, setIsRefundSuccessDialogOpen] =
    useState(false);
  ...
  const defaultStatus = useMemo<ProcessTicketRequest["status"]>(() => {
    ...
    if (
      PROCESSABLE_STATUSES.includes(
        normalized as ProcessTicketRequest["status"]
      )
    ) {
  ...
  const canSendNewMessage =
    !reachedMessageLimit && !isDetailLoading && !detailError;
  ...
  const validatePayload = () => {
    ...
    if (status !== "InReview" && trimmedNotes.length === 0) {
      setError(
        "Vui lòng nhập ghi chú phản hồi khi phê duyệt / từ chối / hủy yêu cầu."
      );
      return false;
    }
  ...
  const handleMessageImageSelect = async (
    ...
    try {
      const uploadPromises = validFiles.map(async (file) => {
        ...
        formData.append(
          "public_id",
          `ticket_message_${Date.now()}_${Math.random()
            .toString(36)
            .substring(7)}`
        );
  ...
  return (
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-3xl max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Thông tin chi tiết và cập nhật trạng thái yêu cầu hỗ trợ / hoàn
              tiền.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 pb-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <Select
                  value={status}
                  onValueChange={(value) =>
                    setStatus(value as ProcessTicketRequest["status"])
                  }
            ...
            {isRefundRequestApproved && (
              <RefundProcessingPanel
                ...
                onRefundSuccess={() => setIsRefundSuccessDialogOpen(true)}
              />
            )}
      ...
      <AlertDialog
        ...
        onOpenChange={setIsRefundSuccessDialogOpen}
      >
        <AlertDialogContent className="sm:max-w-[400px]">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-center text-gray-700">
              Yêu cầu hoàn tiền đã được xử lý thành công. Tiền sẽ được chuyển về
              tài khoản khách hàng.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogAction
              onClick={() => setIsRefundSuccessDialogOpen(false)}
...
const RefundProcessingPanel = ({
  ...
  const derivedOrderId = useMemo(
    () => extractOrderIdFromTicket(ticket),
    [ticket]
  );
  ...
  const findBankInfo = useMemo(() => {
    return (bankCode?: string | null): SupportedBank | null => {
      ...
      return (
        supportedBanks.find(
          (bank) => bank.bin === bankCode || bank.code === bankCode
        ) || null
      );
    };
  }, [supportedBanks]);
  ...
  useEffect(() => {
    ...
    if (ticket.user?.id) {
      ...
      setBankError(
        "Không tìm thấy thông tin người dùng để truy xuất tài khoản ngân hàng."
      );
    }
        ...
        try {
          ...
          if (response.status && response.data) {
            ...
            const initialForms = response.data.orderDetails.map((detail) => ({
              ...
              include: false,
              requiresSerial: requiresSerialForCategory(
            ...
            initialForms.forEach(async (form) => {
              try {
                ...
                const identityNumbers =
                  await getExportedIdentityNumbersByOrderDetailId(
                    form.orderDetailId
                  );
                  ...
                  prev.map((f) =>
                    f.orderDetailId === form.orderDetailId
                      ? {
                          ...
                          requiresSerial:
                            f.requiresSerial || hasSerialInExported,
                        }
                      : f
                  )
                );
            ...
            setOrderInfo(null);
            setOrderError(
              response.errors?.[0] || "Không thể tải thông tin đơn hàng."
            );
          }
        } catch (err: any) {
          setOrderInfo(null);
  ...
  const deliveredAt = orderInfo?.deliveredAt
    ? new Date(orderInfo.deliveredAt)
    : null;
  ...
  const refundEligible =
    Boolean(orderInfo) && deliveredWithin7Days && isDeliveredStatus;
  ...
  const validateRefundPayload = (): string | null => {
    ...
    if (!selectedBankId)
      return "Vui lòng chọn tài khoản ngân hàng để hoàn tiền.";
    if (!selectedDetails.length)
      return "Vui lòng chọn ít nhất một sản phẩm để hoàn.";
    ...
    for (const detail of selectedDetails) {
      ...
      // Kiểm tra xem sản phẩm có số seri trong exportedIdentityNumbers không
      const hasSerialInExported = hasSerialNumbers(
        detail.exportedIdentityNumbers
      );
      ...
      if (mustHaveSerial) {
        ...
        const uniqueSerials = new Set(
          detail.serialNumbers.filter((sn) => sn.trim())
        );
        if (
          uniqueSerials.size !==
          detail.serialNumbers.filter((sn) => sn.trim()).length
        ) {
  ...
  const handleSubmitRefund = async () => {
    ...
    const payload: ProcessRefundRequestPayload = {
      ...
      gatewayPaymentId:
        refundMode === "manual" ? gatewayPaymentId.trim() : null,
    ...
    try {
      ...
      if (response.status) {
        // response.data có thể là object transaction, chỉ set message string
        setSubmitSuccess("Đã gửi yêu cầu hoàn tiền thành công.");
        setSubmitError("");
        // Gọi callback để hiển thị dialog thành công ở component cha
        if (onRefundSuccess) {
          onRefundSuccess();
        }
        setDetailForms((prev) =>
    ...
    <div className="rounded-xl border border-emerald-200 bg-emerald-50/60 p-4 space-y-4">
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-emerald-800">
            Đơn hàng phải đã giao và trong vòng 7 ngày để được hoàn. Vui lòng
            xác thực thông tin ngân hàng trước khi chuyển.
          </p>
        </div>
      </div>
      ...
      <div className="rounded-lg border border-white bg-white/80 p-4 space-y-3">
            ...
            <div>
              ...
              <p className="font-semibold text-gray-900">
                {orderInfo.deliveredAt
                  ? formatDateTime(orderInfo.deliveredAt)
                  : "Chưa xác định"}
              </p>
            </div>
            <div>
        ...
        {!refundEligible && orderInfo && (
          <p className="text-xs text-red-600 bg-red-50 border border-red-100 rounded-md p-2">
            Đơn hàng phải ở trạng thái "Delivered" và chưa quá 7 ngày kể từ ngày
            giao để được hoàn tiền.
          </p>
        )}
      </div>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-500 mt-1">
                        {bankInfo
                          ? bankInfo.name
                          : account.bankCode || "Không xác định"}
                      </p>
                      {!account.isActive && (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div>
                ...
                <p className="mt-1 text-xs text-gray-500">
                  Hệ thống tự tính: {formatCurrency(autoCalculatedAmount)} (chỉ
                  gồm giá sản phẩm, không bao gồm phí vận chuyển). Tối đa:{" "}
                  {formatCurrency(orderInfo?.totalAmount || 0)}
                </p>
              </div>
              <div>
                ...
                <Select
                  value={refundMode}
                  onValueChange={(value: "payos" | "manual") =>
                    setRefundMode(value)
                  }
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="manual">
                      Hoàn tiền thủ công (đã chuyển tay)
                    </SelectItem>
                  </SelectContent>
                </Select>
              </div>
                ...
                <div className="flex items-center gap-2">
                  <Switch
                    ...
                    onCheckedChange={(checked) =>
                      handleDetailToggle(detail.orderDetailId, checked)
                    }
                  />
                        ...
                        detail.exportedIdentityNumbers.forEach((item) => {
                          if (item.lotNumber) {
                            if (item.serialNumber) {
                              ...
                              const current =
                                lotNumberMap.get(item.lotNumber) || 0;
                    ...
                    {detail.identityNumbersLoading ? (
                      ...
                      (() => {
                        const availableSerialNumbers =
                          detail.exportedIdentityNumbers.filter(
                            (item) => item.serialNumber
                          );
                        // Filter theo lot number nếu đã chọn
                        const filteredSerialNumbers = detail.lotNumber
                          ? availableSerialNumbers.filter(
                              (item) => item.lotNumber === detail.lotNumber
                            )
                        ...
                        const mustHaveSerial =
                          detail.requiresSerial || hasSerialInExported;
                        ...
                        if (
                          ...
                          return (
                            <div className="space-y-2 mt-1">
                              {detail.quantity > 0
                                ? Array.from({ length: detail.quantity }).map(
                                    (_, index) => {
                                      // Lấy các số seri đã chọn ở các vị trí khác để loại trừ
                                      const selectedSerials =
                                        detail.serialNumbers.filter(
                                          (_, i) => i !== index
                                        );
                                      const currentSerial =
                                        detail.serialNumbers[index];
                                      ...
                                      const selectableSerials =
                                        filteredSerialNumbers.filter((item) => {
                                          const serialValue =
                                            item.serialNumber!;
                                          ...
                                          return !selectedSerials.includes(
                                            serialValue
                                          );
                                        });
                                        ...
                                        >
                                          ...
                                          {filteredSerialNumbers.length > 0 ? (
                                            ...
                                            <Input
                                              ...
                                              placeholder={(() => {
                                                const hasSerialInExported =
                                                  hasSerialNumbers(
                                                    detail.exportedIdentityNumbers
                                                  );
                                                const mustHaveSerial =
                                                  detail.requiresSerial ||
                                                  hasSerialInExported;
                                                return mustHaveSerial
                                                  ? "Bắt buộc"
                                                  : "Không bắt buộc";
                                              })()}
                                              className="flex-1"
                              ...
                              {(() => {
                                ...
                                const mustHaveSerial =
                                  detail.requiresSerial || hasSerialInExported;
                                return mustHaveSerial &&
                                  detail.serialNumbers.some(
                                    (sn) => !sn.trim()
                                  ) ? (
                                  <p className="text-xs text-amber-600">
                                    Vui lòng chọn đầy đủ số seri cho tất cả sản
                                    phẩm.
                                  </p>
                                ) : null;
                              })()}
                              {detail.lotNumber &&
                                filteredSerialNumbers.length === 0 && (
                                  <p className="text-xs text-gray-500">
                                    Không còn số seri nào khả dụng cho số lô "
                                    {detail.lotNumber}".
                                  </p>
                                )}
                            </div>
