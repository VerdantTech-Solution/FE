...
import { ArrowLeft, ThermometerSun, Droplets, CloudRain, MapPin, Edit } from "lucide-react";
...
const SuggestionItem: React.FC<{ title: string; subtitle: string; priority?: "high" | "medium" | "low"; done?: boolean }>
  = ({ title, subtitle, priority = "medium", done }) => {
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 mt-[80px]">
          ...
          <Card className="mt-4">
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-3">
                {["Hôm nay","Thứ 6","Thứ 7","CN","Thứ 2","Thứ 3","Thứ 4"].map((d, i) => (
                  <div key={i} className={`p-4 rounded-xl border ${i===0?"bg-emerald-50 border-emerald-200":"bg-white"}`}>
                    <div className="text-xs text-gray-500">{d}</div>
                    <div className="mt-2 text-2xl font-semibold text-gray-900">{i===0?"32°":"29°"}</div>
                    <div className="mt-2 grid grid-cols-3 gap-2 text-xs text-gray-600">
                      <div className="flex items-center gap-1"><ThermometerSun className="h-3.5 w-3.5"/>52%</div>
                      <div className="flex items-center gap-1"><Droplets className="h-3.5 w-3.5"/>64%</div>
                      <div className="flex items-center gap-1"><CloudRain className="h-3.5 w-3.5"/>8%</div>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="rounded-xl border p-4 bg-white">
                <div className="text-sm font-medium text-gray-900">Thời tiết hôm nay – Chỉ tiết</div>
                <div className="text-sm text-gray-600 mt-1">Trời nắng nhẹ có mây. Thích hợp để tưới nước vào sáng sớm và chiều mát.</div>
              ...
              <div className="rounded-xl border p-4 bg-amber-50 text-amber-800 border-amber-200 text-sm">Cảnh báo mưa vào thứ 2. Nên chuẩn bị biện pháp che chắn cho cây trồng và kiểm tra hệ thống thoát nước.</div>
            </CardContent>
          </Card>
        </TabsContent>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mt-6">
        <Card className="lg:col-span-2">
          ...
          <CardContent className="space-y-3">
            <div className="flex items-center gap-2 text-sm text-gray-700"><MapPin className="h-4 w-4"/>123 Đường Nông Nghiệp, Xã Tân Thành, Huyện Châu Thành, Tỉnh An Giang</div>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <StatItem label="Diện tích" value="5.5 hecta" />
              <StatItem label="Trạng thái" value="Đang hoạt động" />
              <StatItem label="Cây trồng chính" value="Lúa, Ngô, Rau xanh" />
              <StatItem label="Cập nhật" value="10/01/2024" />
            </div>
            <Separator />
            <div className="flex items-center gap-2 text-xs">
              <span className="px-2 py-1 rounded-md bg-emerald-50 text-emerald-700 border border-emerald-200">Lúa</span>
              <span className="px-2 py-1 rounded-md bg-indigo-50 text-indigo-700 border border-indigo-200">Ngô</span>
              <span className="px-2 py-1 rounded-md bg-lime-50 text-lime-700 border border-lime-200">Rau xanh</span>
            </div>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent className="space-y-2 text-sm text-gray-700">
            <div className="flex items-center justify-between"><span>Quy mô</span><span>Vừa</span></div>
            <div className="flex items-center justify-between"><span>Đã đăng ký cộng</span><span>3 tháng</span></div>
            <div className="flex items-center justify-between"><span>Trạng thái</span><span className="text-emerald-600 font-medium">Hoạt động</span></div>
          </CardContent>
        </Card>
      </div>
    </div>
